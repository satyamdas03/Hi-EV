...
@patch("ev.research.search.httpx.AsyncClient")
@patch("ev.tools.research_tool.LLMClient")
async def test_research_tool_includes_memory_context(mock_llm_client, mock_async_client, store):
    await store.upsert_document_chunks([
        {"source": "user_memory", "source_id": "m2", "chunk_index": 0,
         "text": "RoboCAD is now in Phase 23 focused on humanoid synthesis.", "project_name": "robocad"}
    ])
    ...
    llm_instance = MagicMock()
    llm_instance.complete = AsyncMock(return_value="MPC applies to humanoid robots.")
    mock_llm_client.return_value = llm_instance
    ...
    assert "humanoid synthesis" in result["memory_snippets"][0]
    prompt = llm_instance.complete.call_args.kwargs["messages"][0]["content"]
    assert "user has previously noted" in prompt
    assert "humanoid synthesis" in prompt
